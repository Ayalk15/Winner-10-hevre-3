...
export default function App() {
  ...
  const loginAdmin = async () => {
    if (!ADMIN_PASS_HASH) { flash('חסרה הגדרת סיסמת מנהל ב-Vercel'); return; }
    const enteredHash = await sha256(adminPass);
    if (enteredHash === ADMIN_PASS_HASH) { update({ adminOpen: true }); setAdminPass(''); flash('מצב מנהל הופעל'); }
    else flash('סיסמת מנהל שגויה');
  };
