...
import { Trophy, Home, Users, BarChart3, Shield, Settings, Info, Plus, Trash2, LogOut, BookOpen } from 'lucide-react';

const VERSION = 'Premium v8 — חוקים + קבוצה אהודה';
...
export default function App() {
  ...
  return <div className="app"><div className="shell">
    <header className="top">
      ...
      <nav className="nav">
        {[['home',Home,'בית'],['games',Trophy,'משחקים'],['league',Users,'טבלה'],['stats',BarChart3,'סטט׳'],['profile',Shield,'אישי'],['rules',BookOpen,'חוקים'],['admin',Settings,'מנהל']].map(([id,Icon,label]) => <button key={id} className={tab===id?'active':''} onClick={() => setTab(id)}><Icon size={15}/><br/>{label}</button>)}
      </nav>
    </header>
    ...
    {tab === 'home' && <section className="card hero">
      ...
      <p className="muted">משתתפים, ניחושים, ג׳וקר, ניקוד, קבוצה אהודה, חוקים, טבלה, מצב מנהל ושמירה בדפדפן. כרגע הנתונים נשמרים במכשיר שלך. בשלב הבא נחבר Firebase כדי שכל החברים יראו אותו דבר.</p>
      ...
      <div className="quickActions">
        ...
        <button className="primary secondary" onClick={() => setTab('profile')}>❤️ בחירת קבוצה שאני אוהד</button>
        <button className="primary secondary" onClick={() => setTab('rules')}>📘 חוקי המשחק והניקוד</button>
      </div>
    </section>}
    ...
    {tab === 'games' && <section>
      <div className="card">
        ...
        <div style={{height:10}} />
        <label className="small muted">הקבוצה שאני אוהד</label>
        <select className="select" value={currentTournament.favoriteTeam || ''} onChange={e=>setTournaments(p=>({...p,[currentUser.id]:{...currentTournament,favoriteTeam:e.target.value}}))}><option value="">בחר קבוצה</option>{Object.keys(teams).map(t=><option key={t} value={t}>{teams[t]} {t}</option>)}</select>
        <div style={{height:10}} />
    ...
    {tab === 'league' && <section className="card"><h2>🏆 טבלת דירוג</h2><table><thead><tr><th>#</th><th>משתתף</th><th>אהדה</th><th>נק׳</th><th>בולים</th></tr></thead><tbody>{leaderboard.map((u,i)=><tr key={u.id}><td className="medal">{i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}</td><td>{u.avatar} {u.name}</td><td>{tournaments[u.id]?.favoriteTeam || '—'}</td><td>{u.points}</td><td>{u.exact}</td></tr>)}</tbody></table></section>}
    ...
    {tab === 'profile' && <section className="card"><h2>👤 אזור אישי — כאן בוחרים קבוצה אהודה</h2><div className="participant"><div className="avatar">{currentUser.avatar}</div><div><b>{currentUser.name}</b><div className="small muted">PIN: {currentUser.pin}</div></div><span className="pill">פעיל</span></div><label className="small muted" style={{display:'block',marginTop:12}}>הקבוצה שאני אוהד</label><select className="select" value={currentTournament.favoriteTeam || ''} onChange={e=>setTournaments(p=>({...p,[currentUser.id]:{...currentTournament,favoriteTeam:e.target.value}}))}><option value="">בחר קבוצה</option>{Object.keys(teams).map(t=><option key={t} value={t}>{teams[t]} {t}</option>)}</select><input value={currentTournament.champion} onChange={e=>setTournaments(p=>({...p,[currentUser.id]:{...currentTournament,champion:e.target.value}}))} placeholder="האלופה שלי — 40 נק׳" style={{marginTop:12}}/><input value={currentTournament.topScorer} onChange={e=>setTournaments(p=>({...p,[currentUser.id]:{...currentTournament,topScorer:e.target.value}}))} placeholder="מלך השערים שלי — 40 נק׳ + 2 נק׳ לכל גול" style={{marginTop:10}}/><input value={currentTournament.topAssists} onChange={e=>setTournaments(p=>({...p,[currentUser.id]:{...currentTournament,topAssists:e.target.value}}))} placeholder="מלך הבישולים שלי — 50 נק׳" style={{marginTop:10}}/><button className="primary" style={{marginTop:12}} onClick={()=>notify('הפרופיל נשמר')}>שמור פרופיל</button></section>}


    {tab === 'rules' && <section className="card rules"><h2>📘 חוקים וניקוד</h2><div className="ruleBox"><h3>⚽ ניקוד משחקים</h3><ul><li><b>ניחוש כיוון נכון</b> — 2 נקודות. בית / תיקו / חוץ.</li><li><b>תוצאה מדויקת</b> — 6 נקודות סה״כ.</li><li><b>פספוס</b> — 0 נקודות.</li></ul></div><div className="ruleBox"><h3>🃏 חוק הג׳וקר</h3><ul><li>בכל מחזור אפשר לבחור משחק אחד כג׳וקר.</li><li>ג׳וקר מכפיל את הניקוד: כיוון נכון = 4 נק׳, תוצאה מדויקת = 12 נק׳.</li></ul></div><div className="ruleBox"><h3>👑 ניחושים עונתיים</h3><ul><li>ניחוש אלופה נכון — 40 נקודות בסוף העונה.</li><li>ניחוש מלך שערים נכון — 40 נקודות בסוף העונה.</li><li>מלך השערים שבחרת: 2 נקודות לכל גול שהוא מבקיע במהלך העונה.</li><li>ניחוש מלך בישולים נכון — 50 נקודות בסוף העונה.</li></ul></div><div className="ruleBox"><h3>🔒 נעילה ושקיפות</h3><ul><li>בעתיד נוכל לנעול ניחושים אוטומטית 24 שעות לפני מחזור.</li><li>אחרי שהמנהל סוגר משחק, הניקוד מתעדכן בטבלה.</li><li>כרגע הנתונים נשמרים בדפדפן של המכשיר. לחיבור בין כל החברים נחבר Firebase.</li></ul></div></section>}
    ...
    <section className="card"><h3><Info size={18}/> חשוב</h3><p className="muted small">בגרסה v7 הנתונים נשמרים ב־localStorage של המכשיר. כלומר רענון לא מוחק, אבל טלפון אחר לא יראה אותם. לחיבור בין כל החברים נחבר Firebase בשלב הבא.</p></section>
