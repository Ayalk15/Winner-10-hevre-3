...
const FRIENDS = [
  { name: 'אייל אשכנזי', icon: '👑', role: 'מנהל הליגה', vibe: 'אלוף הבית' },
  { name: 'דודו', icon: '👨‍⚕️', role: 'מנתח משחקים', vibe: 'חזק בבולים' },
  { name: 'אביטל', icon: '👩‍⚕️', role: 'מלכת תיקו', vibe: 'X בזמן הנכון' },
  { name: 'דולב', icon: '🪖', role: 'הולך על הפתעות', vibe: 'אוהב 2 בחוץ' },
  { name: 'גל נהג', icon: '🚕', role: 'מומחה מחזורים', vibe: 'תמיד מגיע בזמן' }
];
...
export default function App() {
  ...
  const leaderboard = [
    { name: `אייל אשכנזי${userTeamSuffix}`, points: stats.totalPoints, trend: stats.totalPoints > 0 ? '▲' : '–', icon: '👑' },
    { name: 'דודו', points: Math.max(0, stats.totalPoints - 7), trend: '–', icon: '👨‍⚕️' },
    { name: 'אביטל', points: Math.max(0, stats.totalPoints - 13), trend: '▲', icon: '👩‍⚕️' },
    { name: 'דולב', points: Math.max(0, stats.totalPoints - 19), trend: '▼', icon: '🪖' },
    { name: 'גל נהג', points: Math.max(0, stats.totalPoints - 25), trend: '–', icon: '🚕' }
  ];
      ...
      <div className="sticky top-0 bg-gray-950/80 backdrop-blur-2xl pt-2 pb-3 z-50 max-w-md mx-auto border-b border-yellow-500/10">
        <header className="relative overflow-hidden rounded-3xl border border-yellow-500/20 bg-gradient-to-br from-gray-900 via-gray-950 to-black p-4 text-center shadow-2xl">
          ...
          <div className="relative">
            ...
            <h1 className="text-3xl font-extrabold text-yellow-400 drop-shadow-md">10 חבר'ה - יוספטל</h1>
            <p className="mt-1 text-[11px] font-bold text-gray-400">ליגת ניחושים פרטית · Winner League 26/27</p>
        ...
        {currentTab === 'dashboard' && (
          <div className="space-y-5">
            ...
            <section className="grid grid-cols-2 gap-3">
              {FRIENDS.map((friend) => (
                <div key={friend.name} className="rounded-2xl border border-gray-800 bg-gray-900 p-3 shadow-lg">
